...
class Browser(QMainWindow):
    ...
    def is_extension_malicious(self, js_code, extension_name):
        """Vérifie si le code JS présente des comportements suspects ou malveillants"""
        suspicious_patterns = [
            "eval(",
            "document.cookie",
            "localStorage",
        ...
        matches_count = sum(1 for pattern in suspicious_patterns if pattern in js_code)
        
        # Si le code combine plusieurs indices suspects, on considère l'extension comme potentiellement dangereuse
        if matches_count >= 2:
            reply = QMessageBox.warning(
                self,
                "Alerte de sécurité ⚠️",
                f"Cette extension est peut être dangereuse : {extension_name} voulez vous continuer ?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.No
            )
            return reply == QMessageBox.StandardButton.No  # Retourne True si l'utilisateur choisit d'arrêter (bloquer)
            ...
            with urllib.request.urlopen(req) as response:
                if response.status == 200:
                    ...
                    for file_info in dialog.selected_files:
                        ...
                        # Vérification de sécurité anti-malware
                        if self.is_extension_malicious(js_code, file_name):
                            continue  # Ignore et bloque l'installation si l'utilisateur refuse
    ...
    def register_extension(self, file_path):
        ...
        # Vérification de sécurité anti-malware pour les fichiers locaux également
        if self.is_extension_malicious(js_code, filename):
    ...
    def setup_docks(self):
        ...
        # Bouton explicite pour charger un .js local demandé
        btn_load = QPushButton("📁 Charger un .js local")
        btn_load.setStyleSheet("background-color: #3b82f6; color: white; font-weight: bold; padding: 6px;")
